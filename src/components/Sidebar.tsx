...
export function Sidebar({ children, isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside className={cn(
        "w-64 bg-[#1a1a1a] h-screen flex flex-col fixed left-0 top-0 border-r border-white/5 overflow-hidden z-50 transition-transform duration-300 ease-in-out",
        ...
        <div className="p-6 relative z-10">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-10 h-10 bg-gradient-to-br from-red-600 to-red-800 rounded-xl flex items-center justify-center shadow-lg shadow-red-900/20 shrink-0">
              <span className="text-xl font-bold text-white">{(user?.tenant?.name || 'M').charAt(0)}</span>
            </div>
            <div>
              <h1 className="text-xl font-bold leading-none text-white truncate max-w-[140px]">{user?.tenant?.name || 'MeatMaster'}</h1>
              <span className="text-[10px] font-bold text-red-500 tracking-widest uppercase">SISTEMA DE GERENCIAMENTO</span>
            </div>
          </div>
        </div>
